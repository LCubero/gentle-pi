...
test("v1.0.0 release package and runtime stop before delivery or publication", () => {
	const packageJson = readPackageJson();
	assert.equal(packageJson.version, "1.0.0", "the release manifest must remain explicitly pinned to v1.0.0");
	assert.equal(
