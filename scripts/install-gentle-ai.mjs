...
if (process.env.GENTLE_PI_SKIP_GENTLE_AI_INSTALL === "1") {
	...
	try {
		...
		console.log(`Gentle AI v2.1.4 ${result.installed ? "installed" : "integrity-verified"} at ${result.binaryPath}`);
	} catch (error) {
		console.error(`gentle-pi could not install its package-local Gentle AI v2.1.4 binary: ${error instanceof Error ? error.message : String(error)}`);
		process.exitCode = 1;
	}
}
